...
\begin{tikzpicture}[scale=1]
  ...
  \draw[->](1.75*\X, 0*\Y) -- node[sloped,above]{$locked$} (3*\X, -1*\Y);

  \draw[->](2*\X, -1*\Y)--(2.5*\X, -2*\Y); %% p2 -> m -> p3
  \draw[->](3*\X, -1*\Y)--(3.5*\X, -2*\Y); %% p2 -> l -> p3
  ...
  \draw[->, decorate, decoration={snake, amplitude=0.3mm}](3.5*\X, -2*\Y)--
  node[sloped, above left]{$ack$}(4*\X, 0); %% p3 -> a -> p1
  ...
  %% p1 -> m' -> p1
  \draw[->](2.75*\X, 0pt)node[anchor=south]{$b(m')$} -- (3.9*\X, -1*\Y); 
  \draw[->](3.9*\X, -1*\Y) -- (5.75*\X, -2*\Y); %% p2 -> m' -> p3

  %% p2 -> [m] -> p3
  \draw[->](4.15*\X, 0*\Y)node[anchor=south]{$[m']$} -- (4.4*\X, -2*\Y);
  \draw(4.6*\X, 2 -2*\Y)--node[anchor=north]{$d(m')$}(4.6*\X, -2 - 2*\Y);

  %% p2 -> m'' -> p3
  \draw[->](4.85*\X, 0*\Y) node[anchor=south]{$b(m'')$}-- (5.1*\X, -2*\Y);
  \draw[->](4.85*\X, 0*\Y) -- (6.1*\X, -1*\Y); %% p1 -> m'' -> p2 (++0.35)
  \draw[->](6.1*\X, -1*\Y) -- (6.6*\X, -2*\Y); %% p2 -> m'' -> p3
  \draw(5.4*\X, 2 -2*\Y)--node[anchor=north]{$d(m'')$}(5.4*\X, -2 - 2*\Y);
  \draw(6*\X, 2pt)--node[anchor=south]{$d(m'')$}(6*\X, -2pt);
