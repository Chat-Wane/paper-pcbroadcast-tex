...
\PAR{Piggybacking approaches~\cite{birman1987reliable,hadzilacos1993fault}.}{A
  ...
  size may increase quickly depending on the application. \CBROADCAST does not
  piggyback all preceding messages in broadcast messages. However, an
  accumulation of messages arises during buffering. However, as discussed in
  Section~\ref{subsec:complexity}, we can assume that links quickly become safe
...
\PAR{Vector clock approaches~\cite{fidge1988timestamps,mattern1989virtual}.}{A
  ...
  depends of the desired boundary. \\
  ...
  concurrent or if one precedes the other. Nevertheless, accurate causal delivery
  is a feature provided by default by the propagation scheme. It alleviates
  messages from the need to piggyback heavy control
