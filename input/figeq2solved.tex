...
%%  \draw[fill=black](0pt, -2*\Y) node[anchor=east]{$p_3$}circle(2pt);
  ...
  \draw[->, densely dotted](1.75*\X, 0*\Y) --node[sloped,above]{$locked$} (3.5*\X, -1*\Y);
...
%%  \draw[->, dashed](3*\X, -1*\Y)--(3.5*\X, -2*\Y); %% p2 -> l -> p3
  ...
  \draw[->](4.85*\X, 0pt)node[anchor=south]{$b(m')$} -- (5.1*\X, -1*\Y);
  \draw(6*\X, 2pt)--node[anchor=south]{$d(m')$}(6*\X, -2pt);
  \draw(5.4*\X, 2 - 1*\Y)--node[anchor=north]{$d(m')$}(5.4*\X, -2 - 1 *\Y);
  \draw[->, densely dotted](4.85*\X, 0pt) -- (6.6*\X, -1*\Y); %% p1 -> m' -> p2
