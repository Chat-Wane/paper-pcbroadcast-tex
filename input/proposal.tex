...
\begin{definition}[Safe link]
  ...
  A link from Process~A to Process~B is safe if A delivered messages that
  \begin{itemize}
  \item B delivered, or
  \item B will deliver
  \end{itemize} 
...
Added links start unsafe. The challenge is to make them safe using local
knowledge only. A straightforward mean for Process~A to achieve this consists in
sending all its delivered messages to Process~B using this unsafe link. This
guarantees that any message delivered by A will be delivered by B before A starts
using the new -- now safe -- link for causal broadcast. However, this is costly
both in local space and generated traffic. 
...
knows that all messages it delivered before it sent the signal are delivered by
Process~B; then it guarantees the receipt by B of messages it delivered later by
sending the array of messages using the unsafe link. The link is safe. 
