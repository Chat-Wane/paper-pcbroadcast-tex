...
messages to all other processes in their systems.
...
neighbors from $Q$ at any time. The next section describes \CBROADCAST, a causal
...
Both functions broadcast (see Line~\ref{line:rbroadcast}) and deliver (see
...
%does not reorder messages traveling through the links


%%Algorithm~\ref{algo:bufferbroadcast} does not provide specific
...
Unfortunately, Figure~\ref{fig:preventiveproblem} shows that adding links may
...
\begin{proof}
  ...
  \implies d_C(m) \rightarrow d_C(m')$. It ensures FIFO order when $A=B$.
  It ensures local order when $A \neq B$.\\
  ...
  in $Q$ are safe links, $r_c(m) \rightarrow r_{CB}(m')$. Since delivery order follows first receipt order,
  $d_C(m) \rightarrow d_C(m')$. This transitively reach all correct processes as long as the network remains
  unpartioned.
\end{proof}
