...
a forwarding as parent, we can build a graph using the paths taken by messages
from the origin of the broadcast to any process.
...
\begin{proof} \TODO{Prove validity, uniform agreement, uniform integrity?} To
  ...
  delivers $m$ before $m'$. Either Process $q$ received $m$ before Process $p$
  or not. In the later case, since broadcast and forward have identical
  instructions, the proof is identical to FIFO's one. In the former
  case,
  \begin{inparaenum}[(i)]
...
\begin{proof}
  ...
  \textbf{FIFO:} Suppose a process $p$ broadcasts $m$ before $m'$. Consider that
  a correct process $q$ delivers $m'$. We must show that $q$ delivers $m$ before
  $m'$. If Process $p$ broadcasts $m$ before the link addition, Process $p$
  either broadcast $m'$ during buffer phase or after acknowledgment. In the
  former case, since the receipt of $m$ precedes the creation of the
  acknowledgement, when $p$ 

  \TODO{meow} \\
  \textbf{Local:} Suppose a process $p$ delivers $m$ before broadcasting
  $m'$. Consider a correct process $q$ that delivers $m'$. We must show that $q$
  delivers $m$ before $m'$. \TODO{miaw} \TODO{The only difference between FIFO and  Local is that Process q may have received m before p broadcast m'} \\
