...
\begin{definition}[Safe link]
  ...
  \TODO{$safe(AB) \equiv \forall m,\, m',\, d_A(m) \rightarrow s_{AB}(m') \implies
  r_B(m) \rightarrow r_{AB}(m')$}

  % \TODO{More general:
  %   $safe(AB) \equiv \forall m,\, m',\, d_A(m) \rightarrow s_{AB}(m') \implies
  %   r_B(m) \rightarrow r_{B}(m')$}

  %   \TODO{Should they be only received by B instead of delivered ? Less
  %   restrictive and we find FIFO links:
  %   $\implies r_B{m} \rightarrow r_AB(m')$.}
\end{definition}
...
knows that all messages it delivered before the signal are received by
...
\begin{lemma}[Ping and buffering makes safe links]
  Process~A makes an unsafe link to Process~B safe by sending a signal to B via
  safe links while buffering its delivered messages. A link is safe once
  Process~A sent buffered messages after receiving B's acknowledgement of the
  signal. \\
  \TODO{$\forall i,\, m_i \rightarrow m_{i+1},\,
    ack(m_i) \rightarrow \forall j>i,\, s_{AB}(m_j) \rightarrow s_{AB}(m_{j+1}) \implies safe(AB)$}
\end{lemma}
...
\begin{proof}
  ...
  % We must show that once Process~A sent its buffer using the new link, the
  % latter becomes safe. \\
  ...
  % If $m'$ comes from the buffer, $m$ 
  % Process~A delivers $m_1 \ldots m_i$ then
  % pings Process~B using safe links. Since the network does not have partitions,
  % Process~B receives $m_1 \ldots m_i$ and the ping. Since links are FIFO,
  % Process~B receives $m_1 \ldots m_i$ before the ping. Process~B replies to
  % Process~A. In the meantime, Process~A delivers and buffers $m_j \ldots
  % m_k$.
  % Process~A receives Process~B's reply after delivering $m_k$ and before
  % delivering $m_\ell$. Process~A receives Process~B's reply after Process~B
  % received $m_1 \ldots m_i$.  
  %  Process~A sends $m_j \ldots m_k$ using the unsafe
  % link. \\
  % Suppose Process~A delivers $m_\ell \ldots m_m$ and sends $m_\ell \ldots m_m$
  % using the new link. If the link is unsafe   
  % Process~B already received $m_1 \ldots m_i$ and since
  % links are FIFO, Process~B will receive $m_j \ldots m_k$ before receiving
  % $m_\ell \ldots m_m$.
\end{proof}
...
% \begin{lemma}[Safe links stay safe]
%   Once a link is safe, it stays safe until its removal.
% \end{lemma}

% \begin{proof}
%   Links are FIFO. Using such links, processes receive messages in the order they
%   were sent. This guarantees that a message $a$ sent before a message $a'$ will
%   be delivered before $a'$
% \end{proof}
