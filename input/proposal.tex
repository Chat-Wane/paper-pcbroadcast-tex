...
\begin{definition}[Safe link]
  ...
  % \TODO{$safe(AB) \equiv \forall m,\, m',\, d_A(m) \rightarrow s_{AB}(m')
...
\begin{definition}[Ping phase]
  Ping phase starts when Process~A pings Process~B. Ping messages travel using
  safe links. When Process~B receives this ping, it replies to
  Process~B. Replies travel using any communication mean. Ping phase ends when
  Process~A receives the reply of Process~B.
\end{definition}
...
\begin{proof}
  ...
  by Process~A. Process~A receives Process~B replies after Process~B received
...
% The ping phase ensures that Process~D received all messages preceding $a$ and
% $a$ itself. The buffering phase ensures that Process~D receives all messages
% between the sending of the ping message $\pi$ and the receipt of Process~D's
% reply $\rho$. Afterwards, Process~D will receive Process~A's broadcast or
% forwarded messages from this new direct link or transitively through Process~B.
...
\begin{proof}
  ...
  delivers $m$ before $m'$. \\
  Since processes use only safe links to disseminate messages, when broadcasting
  $m'$, direct neighbors already received or are guaranteed to receive $m$
  before. Since delivery order follows the receipt order, direct neighbors
  deliver $m$ before $m'$. Then, they forward $m$ and $m'$ in this
  order. \TODO{meow}
