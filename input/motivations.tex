...
\begin{figure*}
  \begin{center}
    \subfloat[Part A][\label{fig:generalsolveA}Process~A broadcasts $b$.]
    {\input{input/figgeneralproblemA.tex}}
    \hspace{20pt}
    \subfloat[Part B][\label{fig:generalsolveB}Process~B receives and 
    delivers $a$. Process~B forwards $a$ using FIFO links.]
    {\input{input/figgeneralsolveA.tex}}
    \hspace{20pt}
    \subfloat[Part C][\label{fig:generalsolveC}Process~C cannot receive $b$
    without having received $a$ beforehand. It ensures causal delivery.]
    {\input{input/figgeneralsolveB.tex}}
    \hspace{20pt}
    \subfloat[Part D][\label{fig:generalsolveD}meow.]
    {\input{input/figgeneralsolveC.tex}}
    \caption{\label{fig:generalsolve}Preventive broadcast principle ensures
      causal order.}
  \end{center}
\end{figure*}
...
Figure~\ref{fig:generalsolveA} shows that when Process~B receives and delivers
