...
\begin{figure*}
  \begin{center}
    ...
    \hspace{20pt}
    \subfloat[Part B][Process~B receives and delivers $a$ then broadcasts $b$.]
    {\input{input/figgeneralproblemB.tex}}
    \hspace{20pt}
    \subfloat[Part C][Process~C receives $b$ before $a$. The immediate delivery
    of $b$ violates causal order.]
    {\input{input/figgeneralproblemC.tex}}
    \caption{Causal order violation in network without causal broadcast.}
  \end{center}
\end{figure*}
...
\begin{figure}
  \begin{center}
    ...
    \caption{Upon receipt of $a$, Process~B forwards $a$ 
    using FIFO links to all its neighbors. Process~C cannot receive $b$
...
\begin{figure*}
  \begin{center}
    % \subfloat[Small network][Upon receipt of $a$, Process~B forwards $a$ 
    % using FIFO links to all its neighbors. Process~C cannot receive $b$
    % without having received $a$ beforehand.]
    % {\input{input/figgeneralsolveA.tex}}
    % \hspace{20pt}
%    \subfloat[Large network][It works in larger networks where processes
%    have partial knowledge of the network membership.]
    \input{./input/figdisseminationtree.tex}
    \caption{The principle of preventive causal broadcast works in large
      networks where processes have partial knowledge of the network membership.}
...
\TODO{Talk about overlay networks here. Later on, talk about dynamicity.}

% For instance, conflict-free replicated data
% types~\cite{shapiro2011comprehensive} are replicated data structures ensuring
% eventual consistency~\cite{bailis2013eventual}. Replicas eventually converge to
% an identical state. CRDTs for sets~\cite{mukund2014optimized} and
